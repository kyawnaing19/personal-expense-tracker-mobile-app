...
void _calculateResult() {
 ...
 Widget _buildAppBar() {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          // Calendar UI မှာ လ/နှစ် ကို နှိပ်ပြီး လအရှင် ချိန်းလို့ရမယ့် Logic (image_4a530d.png)
          if (_currentState == CategoryState.calendar)
            GestureDetector(
              onTap: () async {
                // လ/နှစ် ကို နှိပ်လိုက်ရင် Dialog ပွင့်လာပြီး လွတ်လပ်စွာ ရွေးချယ်နိုင်ရန်
                final DateTime? picked = await showDatePicker(
                  context: context,
                  initialDate: _currentSelectedCalendarDate,
                  firstDate: DateTime(2020),
                  lastDate: DateTime(2030),
                  initialDatePickerMode: DatePickerMode.year, // နှစ်နဲ့ လကို အဓိကရွေးခိုင်းခြင်း
                  helpText: "SELECT MONTH & YEAR",
                );
                if (picked != null) {
                  setState(() {
                    // ရွေးချယ်လိုက်တဲ့ လ/နှစ်အတိုင်း Calendar Grid ထဲမှာ ရက်တွေ Auto ပြောင်းသွားပါမယ်
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    // ပုံပါအတိုင်း အပေါ်အောက်မျှားပုံစံလေး ထည့်ထားပါတယ်
    ...
    Text(_amount, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF111827))),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: IconButton(
                  icon: Icon(_pickedImage != null ? Icons.check_circle : Icons.camera_alt_outlined, 
                             color: _pickedImage != null ? const Color(0xFF10B981) : const Color(0xFF4B5563)),
                  onPressed: _showImageSourceDialog,
                ), 
                enabledBorder: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(4),
  ...
  Widget _buildBalancedKeypadUI() {
    return LayoutBuilder(
      builder: (context, constraints) {
        double cellHeight = 44; // Button height

        return GridView.count(
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          crossAxisCount: 4, // 4 columns layout as in image_490f9a.png
          mainAxisSpacing: 8,
          crossAxisSpacing: 10,
          childAspectRatio: (constraints.maxWidth / 4) / cellHeight,
          children: [
...
Widget _buildActionButton(String label) {
    ...
    Color btnColor = const Color(0xFFEDE9FE); // Default light purple color
    Color textColor = Colors.black;

    if (isConfirm) {
      btnColor = hasValue ? const Color(0xFF10B981) : const Color(0xFFEDE9FE);
    ...
    return GestureDetector(
      onTap: () {
        ...
        if (isConfirm && _currentOperator != null) {
          _calculateResult();
        }
  ...
  Widget _buildCalendarView(double screenWidth) {
    ...
    // --- Real-world Dynamic Calendar Logic ---
          ...
          Expanded(
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
  key: ValueKey(cellDate.toString()), //  key ကို GestureDetector ဆီ ရွှေ့ပေးလိုက်ပါပြီ
  onTap: () {
    setState(() {
