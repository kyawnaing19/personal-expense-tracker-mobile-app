...
class _RecordHistoryScreenState extends State<RecordHistoryScreen> {
  ...
  // ---- Pagination: screen ကနေ တစ်ခါထဲ item အားလုံးကို build မလုပ်တော့ဘဲ
  // ၁၀ ခုစီ ခွဲပြီး scroll ဆင်းလာတာနဲ့ ထပ်ဖြည့်ပေးမှာဖြစ်လို့ data ပေါ်တာမြန်ပါတယ်။
      ...
      body: SafeArea(
        child: BlocConsumer<TransactionBloc, TransactionStateBase>(
          ...
          builder: (context, state) {
            ...
            return Column(
              children: [
                // Header (title, All/Expense/Income tabs, filter & calendar icons)
                // ကို scroll ထဲ မထည့်တော့ဘဲ ဒီနေရာမှာ static ပြထားပါတယ် -- အောက်ကို
                // ဆွဲလိုက်ရင်လည်း ပျောက်မသွားတော့ပါ။
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 16.0),
                  child: Column(
                    children: [
            ...
            itemBuilder: (context, idx) {
              ...
              return GestureDetector(
                ...
                onTap: () async {
                  ...
                  // Reload မလိုတော့ပါ -- transaction ကို ပြင်/ဖျက်ခဲ့ရင်
                  // TransactionBloc က TransactionActionSuccess ကို emit လုပ်ပြီးသား
                  // ဖြစ်လို့ အပေါ်က BlocConsumer listener က အလိုအလျောက်
                  // LoadTransactions() ခေါ်ပေးမှာဖြစ်ပါတယ်။ ဘာမှမပြင်ဘဲ ပြန်ခဲ့ရင်လည်း
                  // ထပ်ခါထပ်ခါ API မခေါ်တော့ပါ။
                },
                child: Row(
                  children: [
                    CircleAvatar(
          ...
          child: Container(
            ...
            child: Row(
              children: ['All', 'Expense', 'Income'].map((tab) {
                ...
                return Expanded(
                  child: GestureDetector(
                    onTap: () => setState(() { _selectedFilter = tab; _visibleTransactionCount = _pageSize; }),
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 8),
                      decoration: BoxDecoration(color: isSelected ? primaryPurple : Colors.transparent, borderRadius: BorderRadius.circular(8)),
                      child: Text(tab, textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w600, color: isSelected ? Colors.white : Colors.grey)),
